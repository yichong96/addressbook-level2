...
Finds persons whose names contain any of the given keywords or whose keywords are substrings of names +
...
* `find Joh` +
Returns any person whose names contain Joh. Returns `John Doe` in this case. 

* `find Joh Betsy Tim`  + 
Returns `Betsy` and `Tim`. Arguments with keyword matching names and substrings of names mixed togeher will show persons whose names matches with the keyword only. 
